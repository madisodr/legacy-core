...
bool ObjectMgr::InsertCreatureTemplate(uint32 entry)
{
    //                                                 0              1                 2                  3                 4            5           6        7         8
    QueryResult result = WorldDatabase.PQuery("SELECT entry, difficulty_entry_1, difficulty_entry_2, difficulty_entry_3, KillCredit1, KillCredit2, modelid1, modelid2, modelid3, "
       ...
       "d ynamicflag family, trainer_type, trainer_spell, trainer_class, trainer_race, minrangedmg, maxrangedmg, rangedattackpower, type, "
    ...
    if (!result)
    {
        TC_LOG_ERROR("sql.sql", "Creature reloading FAILED!");
        return false;
    }
    ...
    creatureTemplate.ScriptID = sObjectMgr->GetScriptId(fields[80].GetCString());

    TC_LOG_INFO("misc", "Creature %u has been inserted into memory.", entry);
...
void ObjectMgr::DeleteCreatureTemplate(uint32 entry)
{
    if (_creatureTemplateStore.find(entry) == _creatureTemplateStore.end())
    {
        TC_LOG_ERROR("server.loading", "Creature not found in data store. Aborting procedure.");
        return;
    }

    _creatureTemplateStore.erase(entry);
}
...
void ObjectMgr::LoadCreatures()
{
    ...
    do
    {
        ...
        if (data.itemEntry1 != 0)
        {
            if (!sObjectMgr->GetItemTemplate(data.itemEntry1))
            {
                TC_LOG_ERROR("sql.sql", "table `creature` have creature (GUID: %u Entry: %u) with `itemEntry1`=%u (nonexistent item), set to 0", guid, data.id, data.itemEntry1);
                data.itemEntry1 = 0;
            }
        }

        if (data.itemEntry2 != 0)
        {

            if (!sObjectMgr->GetItemTemplate(data.itemEntry2))
            {
                TC_LOG_ERROR("sql.sql", "table `creature` have creature (GUID: %u Entry: %u) with `itemEntry2`=%u (nonexistent item), set to 0", guid, data.id, data.itemEntry2);
                data.itemEntry2 = 0;
            }
        }

        if (data.itemEntry3 != 0)
        {
            if (!sObjectMgr->GetItemTemplate(data.itemEntry3))
            {
                TC_LOG_ERROR("sql.sql", "table `creature` have creature (GUID: %u Entry: %u) with `itemEntry3`=%u (nonexistent item), set to 0", guid, data.id, data.itemEntry3);
                data.itemEntry3 = 0;
            }
        }
