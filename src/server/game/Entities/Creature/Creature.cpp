...
bool Creature::InitEntry(uint32 entry, CreatureData const* data /*= nullptr*/)
{
	...
	{
		if (data->itemEntry1)
			SetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_ID + 0, data->itemEntry1);
		if (data->itemEntry2)
			SetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_ID + 1, data->itemEntry2);
		if (data->itemEntry3)
			SetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_ID + 2, data->itemEntry3);
	}
...
void Creature::Update(uint32 diff)
{
    if (IsAIEnabled && TriggerJustRespawned)
    {
        ...
        if (m_vehicleKit)
            m_vehicleKit->Reset();
		if (m_creatureData->faction != 0)
			setFaction(m_creatureData->faction);
    }
...
void Creature::SaveToDB(uint32 mapid, uint8 spawnMask, uint32 phaseMask)
{
    ...
    uint8 index = 0;
	// todo - change prepared statements to phase out manual worldexecutes -pendragon
