...
public:
	...
	/*******************************************
	* EDITS MADE FOR LEGACY GO BELOW HERE
	* Themistocles
	* Pendragon
	**/
	...
	void LoadDeathSystem();
	void SaveDeathSystem();
	/*
	uint32 GetDeaths() { return l_deaths; }
	uint32 GetInjuryPoints(){ return l_injuryPoints.size(); }
	bool canPlayerExecute(){ return ((l_executionTimer <= time(NULL)) ? true : false); }

	void SetDeaths(bool gain);
	void ApplyInjury(uint32 effect);
	bool hasInjury(uint32 injury);

	void RemoveInjury(Injury* i);
	void RemoveInjuryPoint(InjuryPoint* i);
	*/
