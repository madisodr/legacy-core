...
struct GameObjectData
{
    explicit GameObjectData() : id(0), mapid(0), phaseMask(0), posX(0.0f), posY(0.0f), posZ(0.0f), orientation(0.0f),
                                ...
                                animprogress(0), go_state(GO_STATE_ACTIVE), spawnMask(0), artKit(0), size(1.0f), owner(0), dbData(true) { }
...
class GameObject : public WorldObject, public GridObject<GameObject>, public MapObject
{
    public:
		...
		// LEGACY STYFF
		float m_scale;
		float getObjectScale(){ return m_scale; }
		void setObjectScale(float size); // not quite an override of Object::SetObjectScale()

		uint32 objectOwner;
		void SetObjectOwner(uint32 guid);
		uint32 GetObjectOwner();
		bool ownerExists();

		/* LEGACY APT METHODS AND VARIABLES */
		void loadAptInfo();
		bool isApt;
		bool isPlaced;
