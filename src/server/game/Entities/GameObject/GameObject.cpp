...
void GameObject::DeleteFromDB()
{
	...
	/* APT system */
	QueryResult result = WorldDatabase.PQuery("DELETE FROM legacy_gameobject WHERE guid='%u'", m_DBTableGuid);
}
    ...
    switch (GetGoType())
    {
        ...
        case GAMEOBJECT_TYPE_CHAIR:                         //7
        {
            ...
            for (ChairSlotAndUser::iterator itr = ChairListSlots.begin(); itr != ChairListSlots.end(); ++itr)
            {
				...
				float relativeDistance = (GetGOData()->size*itr->first) - (GetGOData()->size*(info->chair.slots - 1) / 2.0f);
...
/* Legacy Themistocles */
void GameObject::SetObjectOwner(uint32 guid)
{
	uint32 owner = guid;
	QueryResult result = WorldDatabase.PQuery("INSERT INTO legacy_gameobject (guid, owner) VALUES ('%u', '%u') ON DUPLICATE KEY UPDATE owner=VALUES(owner)", GetGUIDLow(), owner);
}

/* Legacy Themistocles */
uint32 GameObject::GetObjectOwner()
{
	uint32 owner;
	QueryResult result = WorldDatabase.PQuery("SELECT owner FROM legacy_gameobject WHERE guid='%u'", GetGUIDLow());
...
/* Legacy Themistocles */
bool GameObject::ownerExists()
{
	uint32 owner = GetObjectOwner();

	if (owner == 0)
		return true;

	QueryResult result = CharacterDatabase.PQuery("SELECT account FROM characters WHERE guid='%u'", owner);
	if (result)
	{
		Field* fields = result->Fetch();
...
/* Legacy Themistocles */
void GameObject::setObjectScale(float size)
{
	m_scale = size;
	SetObjectScale(size);
}