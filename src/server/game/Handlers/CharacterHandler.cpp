    ...
    */
    ...
    switch (_charCreateCallback.GetStage())
    {
        case 0:
        {
			...
			* LEGACY - PENDRAGON
			*********************/
			if (createInfo->Class == 6 && seclvl < 2)
			{
        ...
        case 2:
        {
            if (result)
            {
                ...
                if (createInfo->CharCount >= sWorld->getIntConfig(CONFIG_CHARACTERS_PER_REALM) /* LEGACY PENDRAGON ~> */)
                {
                    /* LEGACY PENDRAGON IF BLOCK, CONTENTS ORIGINAL*/
                    if (createInfo->CharCount >= 10)
                    {
