        ...
        static void AddScript(TScript* const script)
        {
            ...
            if (script->IsDatabaseBound())
            {
                ...
                if (id)
                {
                    ...
                    {
						/* all boss scripts were unattached from templates, now console is clogged with these errors. -pendragon*/
...
bool ScriptMgr::OnGossipSelect(Player* player, Item* item, uint32 sender, uint32 action)
{
    ...
    //FOREACH_SCRIPT(PlayerScript)->OnGossipSelect(player, item, sender, action);
}
...
bool ScriptMgr::OnGossipSelectCode(Player* player, Item* item, uint32 sender, uint32 action, const char* code)
{
    ...
    // FOREACH_SCRIPT(PlayerScript)->OnGossipSelectCode(player, item, sender, action, code);
}
// END OF PROLOGUE EDIT=
