    ...
    public:
        ...
        ChatCommand* GetCommands() const override
        {
            ...
            static ChatCommand gobjectSetCommandTable[] =
            {
                ...
                { "scale", rbac::RBAC_PERM_COMMAND_GOBJECT_SET_SCALE, false, &HandleGameObjectSetScaleCommand, "", NULL },
                { NULL,    0,                                   false, NULL,                              "", NULL }
            };
        ...
        static bool HandleGameObjectTargetCommand(ChatHandler* handler, char const* args)
        {
            ...
            if (target)
            {
                ...
                player->SetSelectedGameObject(target);
            }
            return true;
        }
        ...
        static bool HandleGameObjectDeleteCommand(ChatHandler* handler, char const* args)
        {
            if(handler->GetSession()->GetPlayer()->HasSelectedGameObject() == true)
            {
                GameObject* object = handler->GetSession()->GetPlayer()->GetSelectedGameObject();
                WorldDatabase.PQuery("DELETE FROM apt_placed_objects WHERE objectGuid = %u", object->GetGUIDLow());

                object->SetRespawnTime(0);                                 // not save respawn time
                object->Delete();
                object->DeleteFromDB();
                handler->GetSession()->GetPlayer()->DeselectGameObject();
        ...
        static bool HandleGameObjectInfoCommand(ChatHandler* handler, char const* args)
        {
            ...
            if (!*args)
            {
                if (WorldObject* object = handler->GetSession()->GetPlayer()->GetSelectedGameObject())
