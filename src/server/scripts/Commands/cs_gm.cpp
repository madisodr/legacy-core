...
public:
    ...
    static bool HandleGMFlyCommand(ChatHandler* handler, char const* args)
    {
		...
		if (strncmp(args, "on", 3) == 0)
		{
			...
			if (!target->IsFlying())
				CharacterDatabase.PExecute("INSERT INTO character_flying (guid) VALUES (%u)", uint64(target->GetGUID()));
		}
		else if (strncmp(args, "off", 4) == 0)
		{
			data.SetOpcode(SMSG_MOVE_UNSET_CAN_FLY);
			CharacterDatabase.PExecute("DELETE FROM character_flying WHERE guid = %u", uint64(target->GetGUID()));
		}
        else
        {
            handler->SendSysMessage(LANG_USE_BOL);
