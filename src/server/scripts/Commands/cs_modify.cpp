...
public:
    ...
    static bool HandleModifyASpeedCommand(ChatHandler* handler, const char* args)
    {
		...
		{
			CharacterDatabase.PExecute("INSERT INTO character_speed (guid, speed) VALUES (%u, %f) ON DUPLICATE KEY UPDATE speed = %f", uint64(target->GetGUID()), ASpeed, ASpeed);
		}
		else
		{
			CharacterDatabase.PExecute("DELETE FROM character_speed WHERE guid = %u", uint64(target->GetGUID()));
		}
        return true;
    }
    ...
    static bool HandleModifyScaleCommand(ChatHandler* handler, const char* args)
    {
        ...
        if (Player* player = target->ToPlayer())
        {
			...
			if (Scale == 1.0f)
			{
				CharacterDatabase.PExecute("DELETE FROM character_scale WHERE guid=%u", uint64(player->GetGUID()));
			}
			else
			{
				CharacterDatabase.PExecute("INSERT INTO character_scale (guid, scale) VALUES (%u, %f) ON DUPLICATE KEY UPDATE scale = %f", uint64(player->GetGUID()), Scale, Scale);
			}
        }
    ...
    static bool HandleModifyMorphCommand(ChatHandler* handler, const char* args)
    {
		...
		if (Player* player = target->ToPlayer())
		{
			CharacterDatabase.PExecute("INSERT INTO character_morph (guid, morph) VALUES (%u, %u) ON DUPLICATE KEY UPDATE morph = %u", uint64(player->GetGUID()),
...
//demorph player or unit
    static bool HandleDeMorphCommand(ChatHandler* handler, const char* /*args*/)
    {
		...
		if (Player* player = target->ToPlayer())
		{
			CharacterDatabase.PExecute("DELETE FROM character_morph WHERE guid = %u", uint64(player->GetGUID()));
		}
